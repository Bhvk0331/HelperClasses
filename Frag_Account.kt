...
import com.activityhub.utils.extension.*
           ...
           Glide.with(appcontroller.getAppContext())
                ...
                .listener(object : RequestListener<String, GlideDrawable> {
                    ...
                    override fun onException(e: Exception?, model: String?, target: com.bumptech.glide.request.target.Target<GlideDrawable>?, isFirstResource: Boolean): Boolean {
                        ivYourAccountFragmentProfile.setImageResource(R.drawable.ic_profile_default)
                        progress_profile.visibility = View.GONE
                        Log.e("onException", "e" + e?.printStackTrace())

                        return false
                    }

                })
                .error(R.drawable.ic_profile_default).into(ivYourAccountFragmentProfile)
        ...
        when (apiResponseManager.type) {
            ...
            LOGOUT -> {
                ...
                if (commonResponse.statusCode == STATUS_CODE) {
                    ...
                    appcontroller.setHomePosition(-1)
                    Toast(commonResponse.message, true, activity!!)
                    ...
                    Toast(activity!!.resources.getString(R.string.try_after_some_time), true, activity!!)
                }
            }
        }
...
//                    loadImage(selectedImageUri.toString(), activity!!,ivYourAccountFragmentProfile, R.mipmap.ic_launcher)
                ...
                try {
                    ...
                    ivYourAccountFragmentProfile.setImageBitmap(createBitmap(1, imageStoragePath!!))

                    /*Glide.with(activity).load(imageUri)
                            .error(R.mipmap.ic_launcher)
                            .into(ivYourAccountFragmentProfile)*/

                    file = File(imageStoragePath)
                    Log.e(TAG, "Camera Photo Path---->$file")
    ...
    private fun cameraIntent() {
        ...
        val fileUri = getOutputMediaFileUri(activity!!, file!!)
        intent.putExtra(MediaStore.EXTRA_OUTPUT, fileUri)
        startActivityForResult(intent, REQUEST_CAMERA_PROFILE_PIC)
    }
    ...
    private fun call_logout_api() {
        if (isNetWork(activity!!)) {
            Log.e(TAG, "User Id --->" + sessionManager[getString(R.string.id), ""])
            ApiRequest(activity!!,
                    ApiInitialize.initialize(ApiInitialize.MAIN_URL_API).logout(auth_token,
                            sessionManager[getString(R.string.id), ""]), LOGOUT, true, this)
        } else {
            showSnackBar(activity!!.resources.getString(R.string.internet_not_available))
        }
    }
